...
// Alternative approach: Use a more structured parsing method
  Future<List<Event>> getEventsByCityAlternative(String cityName) async {
    final url = _cityUrlMapping[cityName];
    if (url == null) {
      throw ArgumentError('Invalid city name. Please use one of: ${_cityUrlMapping.keys.join(', ')}');
    }
    ...
    try {
      ...
      // Look for event cards with more specific selectors
      final eventCards = document.querySelectorAll('article[data-testid="event-card"], div[data-testid="event-card"], li[data-testid="event-card"]');
      ...
      if (eventCards.isEmpty) {
        ...
        for (var eventElement in eventElements) {
          final event = _parseEventElement(eventElement);
          if (event != null) {
            events.add(event);
          }
        }
        ...
        for (var card in eventCards) {
          final event = _parseEventElement(card);
          if (event != null) {
            events.add(event);
          }
        }
  ...
  Event? _parseEventElement(element) {
    try {
      // Extract core information
      final imageElement = element.querySelector('img');
      final imageUrl = imageElement?.attributes['src'] ?? imageElement?.attributes['data-src'];

      final titleElement = element.querySelector('h3, h2, [data-testid="event-title"]');
      final title = titleElement?.text.trim();

      if (title == null || title.isEmpty) return null;

      // Extract time and location with multiple strategies
      String? time;
      String? location;

      // Strategy 1: Look for data attributes
      final timeElement = element.querySelector('[data-testid="event-date"], [data-event-start-date]');
      if (timeElement != null) {
        time = timeElement.text.trim();
        if (time == null || time.isEmpty) {
          time = timeElement.attributes['data-event-start-date'];
        }
      }

      final locationElement = element.querySelector('[data-testid="event-location"], [data-event-location]');
      if (locationElement != null) {
        location = locationElement.text.trim();
        if (location == null || location.isEmpty) {
          location = locationElement.attributes['data-event-location'];
        }
      }

      // Strategy 2: Parse from structured text
      if (time == null || location == null) {
        final structuredData = _extractStructuredData(element);
        time ??= structuredData['time'];
        location ??= structuredData['location'];
      }

      // Extract event link
      final eventLink = _extractEventLink(element);
      ...
      print('Error parsing event element: $e');
      return null;
    }
  }

  Map<String, String?> _extractStructuredData(element) {
    final result = <String, String?>{
      'time': null,
      'location': null,
    };

    // Get all text nodes, excluding urgency signals
    final textElements = element.querySelectorAll('p, span, div').where((el) {
      final classes = el.classes.join(' ').toLowerCase();
      final text = el.text.trim().toLowerCase();

      return !classes.contains('urgency') &&
          !classes.contains('signal') &&
          !classes.contains('badge') &&
          !text.contains('sales end') &&
          !text.contains('almost full') &&
          !text.contains('selling fast') &&
          el.text.trim().isNotEmpty;
    }).toList();

    for (var el in textElements) {
      final text = el.text.trim();

      if (result['time'] == null && _isTimeFormat(text)) {
        result['time'] = text;
      } else if (result['location'] == null && _isLocationFormat(text)) {
  ...
  String? _extractEventLink(element) {
    final links = element.querySelectorAll('a[href*="/e/"]');
    for (var link in links) {
      final href = link.attributes['href'];
      if (href != null) {
        return href.startsWith('http') ? href : 'https://www.eventbrite.com$href';
      }
    }
    return null;
  }
