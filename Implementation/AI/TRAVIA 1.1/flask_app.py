...
class TravelRecommendationSystem:
    def __init__(self):
        ...
        self.ppo_agent = None
        self.state_dim = 25  # Increased to include metadata features
    ...
    def create_user_feature_vector(self, user_preferences, metadata_preferences=None):
        ...
        if metadata_preferences:
            metadata_features = [
                metadata_preferences.get('calm_preference', 0.0),
                metadata_preferences.get('noisy_preference', 0.0),
                ...
                metadata_preferences.get('family_friendly_preference', 0.0),
            ]
            features.extend(metadata_features)
        else:
            # Add zeros for metadata features if not available
    ...
    def create_business_feature_vector(self, business):
        """Convert business data to feature vector including metadata attributes"""
        ...
        # Add metadata-style attributes
        business_metadata = self.map_business_to_metadata_format(business)
        metadata_features = [
            business_metadata.get('calm', 0),
            business_metadata.get('noisy', 0),
            business_metadata.get('romantic', 0),
            business_metadata.get('good_for_kids', 0),
            business_metadata.get('classy', 0),
            business_metadata.get('casual', 0),
            business_metadata.get('family_friendly_places', 0),
        ]
        features.extend(metadata_features)
    ...
    def content_based_filtering(self, user_preferences, businesses, user_id=None, limit=50):
        """Apply content-based filtering enhanced with learned preferences and metadata"""
        filtered_businesses = []
        ...
        for business in businesses:
            ...
            # METADATA-BASED SCORING - NEW FEATURE
            if metadata_preferences:
                ...
                print(f"🎯 Metadata score for {business.get('name')}: {metadata_score:.2f}")
    ...
    def update_ppo_model(self, user_id, user_preferences):
        """Update PPO model based on user interactions and metadata preferences"""
        interactions = self.get_user_interactions(user_id)
        ...
        for interaction in interactions:
            try:
                ...
                # Enhanced rewards based on metadata compatibility
                base_reward = 2.0 if interaction['interaction_type'] == 'like' else -2.0
                
                # Bonus reward if the interaction aligns with metadata preferences
                if metadata_preferences and interaction['interaction_type'] == 'like':
                    business_dict = dict(interaction)
                    metadata_score = self.calculate_metadata_compatibility_score(business_dict, metadata_preferences)
                    # Add bonus for high metadata compatibility
                    base_reward += metadata_score * 1.0  
        ...
        print(f"Training PPO with {len(states)} interactions for user {user_id} (with metadata preferences)")
    ...
    def predict_business_ranking(self, user_preferences, businesses, user_id=None):
        """Use PPO model to rank businesses with metadata preferences"""
        if not self.ppo_agent:
            return businesses
        
        # Get metadata preferences if user_id is provided
        metadata_preferences = {}
        if user_id:
            metadata_preferences = self.analyze_user_metadata_preferences(user_id)
    ...
    def get_user_liked_posts_metadata(self, user_id):
        """Get metadata of posts that the user has liked"""
        conn = self.get_db_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        try:
            ...
            liked_metadata = cursor.fetchall()
            print(f"Found {len(liked_metadata)} liked posts with metadata for user {user_id}")
            return [dict(row) for row in liked_metadata]
            
        except Exception as e:
            print(f"Error fetching liked posts metadata: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    
    def analyze_user_metadata_preferences(self, user_id):
        """Analyze user's preferences based on liked posts metadata"""
        liked_metadata = self.get_user_liked_posts_metadata(user_id)
        
        if len(liked_metadata) < 2:
            print(f"Not enough liked posts ({len(liked_metadata)}) to analyze preferences for user {user_id}")
            return {}
        
        # Calculate preference scores for each attribute
        metadata_preferences = {
            'calm_preference': 0,
            'noisy_preference': 0,
            'romantic_preference': 0,
            'good_for_kids_preference': 0,
            'classy_preference': 0,
            'casual_preference': 0,
            'family_friendly_preference': 0,
            'preferred_cuisines': {},
            'preferred_price_ranges': {},
            'preferred_locations': {}
        }
        
        total_likes = len(liked_metadata)
        
        # Analyze binary attributes
        for metadata in liked_metadata:
            if metadata.get('calm') == 1:
                metadata_preferences['calm_preference'] += 1
            if metadata.get('noisy') == 1:
                metadata_preferences['noisy_preference'] += 1
            if metadata.get('romantic') == 1:
                metadata_preferences['romantic_preference'] += 1
            if metadata.get('good_for_kids') == 1:
                metadata_preferences['good_for_kids_preference'] += 1
            if metadata.get('classy') == 1:
                metadata_preferences['classy_preference'] += 1
            if metadata.get('casual') == 1:
                metadata_preferences['casual_preference'] += 1
            if metadata.get('family_friendly_places') == 1:
                metadata_preferences['family_friendly_preference'] += 1
            
            # Analyze cuisine preferences
            cuisine = metadata.get('cuisine_type')
            if cuisine:
                metadata_preferences['preferred_cuisines'][cuisine] = \
                    metadata_preferences['preferred_cuisines'].get(cuisine, 0) + 1
            
            # Analyze price range preferences
            price_range = metadata.get('price_range')
            if price_range:
                metadata_preferences['preferred_price_ranges'][price_range] = \
                    metadata_preferences['preferred_price_ranges'].get(price_range, 0) + 1
            
            # Analyze location preferences
            location = metadata.get('location')
            if location:
                metadata_preferences['preferred_locations'][location] = \
                    metadata_preferences['preferred_locations'].get(location, 0) + 1
        
        # Convert counts to preference scores (0-1)
        for key in ['calm_preference', 'noisy_preference', 'romantic_preference', 
                   'good_for_kids_preference', 'classy_preference', 'casual_preference', 
                   'family_friendly_preference']:
            metadata_preferences[key] = metadata_preferences[key] / total_likes
        
        # Sort preferences by frequency
        metadata_preferences['preferred_cuisines'] = dict(sorted(
            metadata_preferences['preferred_cuisines'].items(), 
            key=lambda x: x[1], reverse=True
        ))
        
        metadata_preferences['preferred_price_ranges'] = dict(sorted(
            metadata_preferences['preferred_price_ranges'].items(), 
            key=lambda x: x[1], reverse=True
        ))
        
        metadata_preferences['preferred_locations'] = dict(sorted(
            metadata_preferences['preferred_locations'].items(), 
            key=lambda x: x[1], reverse=True
        ))
        
        print(f"Metadata preferences for user {user_id}:")
        print(f"  Calm: {metadata_preferences['calm_preference']:.2f}")
        print(f"  Romantic: {metadata_preferences['romantic_preference']:.2f}")
        print(f"  Family-friendly: {metadata_preferences['family_friendly_preference']:.2f}")
        print(f"  Top cuisines: {list(metadata_preferences['preferred_cuisines'].keys())[:3]}")
        
        return metadata_preferences
    
    def map_business_to_metadata_format(self, business):
        """Map business attributes to metadata format for compatibility"""
        return {
            'calm': 1 if business.get('ambience_casual') and not business.get('ambience_classy') else 0,
            'noisy': 1 if business.get('noise_level') == 'loud' else 0,
            'romantic': 1 if business.get('ambience_romantic') else 0,
            'good_for_kids': 1 if business.get('good_for_kids') else 0,
            'classy': 1 if business.get('ambience_classy') else 0,
            'casual': 1 if business.get('ambience_casual') else 0,
            'family_friendly_places': 1 if business.get('good_for_kids') else 0,
            'cuisine_type': business.get('fake_cuisine') or business.get('cuisine_types', [None])[0] if business.get('cuisine_types') else None,
            'price_range': business.get('price_range'),
            'location': f"{business.get('city', '')}, {business.get('state', '')}"
        }
    
    def calculate_metadata_compatibility_score(self, business, metadata_preferences):
        """Calculate how well a business matches user's metadata preferences"""
        if not metadata_preferences:
            return 0.5  # Neutral score if no preferences learned
        
        business_metadata = self.map_business_to_metadata_format(business)
        score = 0
        weight_sum = 0
        
        # Binary attribute matching with stronger weights
        binary_attributes = [
            ('calm', 'calm_preference', 0.3),
            ('noisy', 'noisy_preference', 0.25),
            ('romantic', 'romantic_preference', 0.4),
            ('good_for_kids', 'good_for_kids_preference', 0.35),
            ('classy', 'classy_preference', 0.3),
            ('casual', 'casual_preference', 0.25),
            ('family_friendly_places', 'family_friendly_preference', 0.35)
        ]
        
        for attr, pref_key, weight in binary_attributes:
            user_preference = metadata_preferences.get(pref_key, 0)
            business_value = business_metadata.get(attr, 0)
            
            # Strong positive match if both user likes it and business has it
            if user_preference > 0.5 and business_value == 1:
                score += weight * user_preference
            # Mild positive match if user doesn't care much and business doesn't have it
            elif user_preference <= 0.3 and business_value == 0:
                score += weight * 0.3
            # Neutral if business has it but user doesn't prefer it strongly
            elif user_preference <= 0.5 and business_value == 1:
                score += weight * 0.1
            
            weight_sum += weight
        
        # Cuisine matching with high weight
        business_cuisine = business_metadata.get('cuisine_type')
        if business_cuisine and metadata_preferences.get('preferred_cuisines'):
            cuisine_prefs = metadata_preferences['preferred_cuisines']
            total_cuisine_likes = sum(cuisine_prefs.values())
            
            if business_cuisine in cuisine_prefs:
                cuisine_score = cuisine_prefs[business_cuisine] / total_cuisine_likes
                score += 0.5 * cuisine_score  # High weight for cuisine match
                weight_sum += 0.5
                print(f"✅ Cuisine match: {business_cuisine} (score: {cuisine_score:.2f})")
        
        # Price range matching
        business_price = business_metadata.get('price_range')
        if business_price and metadata_preferences.get('preferred_price_ranges'):
            price_prefs = metadata_preferences['preferred_price_ranges']
            total_price_likes = sum(price_prefs.values())
            
            if business_price in price_prefs:
                price_score = price_prefs[business_price] / total_price_likes
                score += 0.3 * price_score
                weight_sum += 0.3
        
        # Location matching (city/state)
        business_location = business_metadata.get('location')
        if business_location and metadata_preferences.get('preferred_locations'):
            location_prefs = metadata_preferences['preferred_locations']
            total_location_likes = sum(location_prefs.values())
            
            if business_location in location_prefs:
                location_score = location_prefs[business_location] / total_location_likes
...
def metadata_insights():
    ...
    # Get user's metadata preferences
